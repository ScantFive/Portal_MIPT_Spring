...
public class AdvertisementRepository implements AdvertisementRep {
  ...
  public Advertisement create(Advertisement advertisement) {
    ...
    try (Connection connection = DatabaseConfig.getConnection()) {
      ...
      try {
        String sql = "INSERT INTO advertisements (id, status, author, type, category, name, price, description, is_favorite, created_at) "
            +
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        return findById(advertisement.getId()).orElseThrow(() -> new RuntimeException(
    ...
    try (Connection connection = DatabaseConfig.getConnection()) {
      ...
      try {
        ...
        return findById(advertisementId)
            .orElseThrow(() -> new RuntimeException("Advertisement not found after adding photo: " + advertisementId));
  ...
  public Advertisement updatePrice(UUID advertisementId, Long price) {
    ...
    try (Connection connection = DatabaseConfig.getConnection()) {
      ...
      // Возвращаем обновленное объявление
      return findById(advertisementId)
          .orElseThrow(() -> new RuntimeException("Advertisement not found after updating price: " + advertisementId));
  ...
  public Advertisement toggleFavorite(UUID advertisementId) {
    ...
    try (Connection connection = DatabaseConfig.getConnection()) {
      ...
      // Возвращаем обновленное объявление
      return findById(advertisementId).orElseThrow(() -> new RuntimeException(
  ...
  private void reorderPhotos(UUID advertisementId, Connection connection) throws SQLException {
    ...
    String updateOrderSql = "WITH ordered AS ( " +
